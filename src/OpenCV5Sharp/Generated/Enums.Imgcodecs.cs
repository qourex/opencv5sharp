// Copyright (c) 2026 Qourex. Licensed under Apache-2.0.
// See LICENSE file in the project root for full license information.
// AUTO-GENERATED FILE — DO NOT EDIT MANUALLY. Generated by generator.py.

namespace OpenCV5Sharp
{
    /// <summary>
    /// Specifies options and constants for Image Metadata Type.
    /// </summary>
    public enum ImageMetadataType : int
    {
        /// <summary>
        /// Unknown option.
        /// </summary>
        Unknown = -1,
        /// <summary>
        /// Exif option.
        /// </summary>
        Exif = 0,
        /// <summary>
        /// Xmp option.
        /// </summary>
        Xmp = 1,
        /// <summary>
        /// Iccp option.
        /// </summary>
        Iccp = 2,
        /// <summary>
        /// Cicp option.
        /// </summary>
        Cicp = 3,
        /// <summary>
        /// Max option.
        /// </summary>
        Max = 3,
    }

    /// <summary>
    /// Specifies image loading color/depth mode options.
    /// </summary>
    public enum ImreadModes : int
    {
        /// <summary>
        /// Loads the image as-is, including alpha channel if present.
        /// </summary>
        Unchanged = -1,
        /// <summary>
        /// Loads the image as a single-channel grayscale image.
        /// </summary>
        Grayscale = 0,
        /// <summary>
        /// Loads the image as a 3-channel BGR color image.
        /// </summary>
        ColorBgr = 1,
        /// <summary>
        /// Loads the image as a 3-channel BGR color image.
        /// </summary>
        Color = 1,
        /// <summary>
        /// Loads 16-bit or 32-bit images when input has corresponding depth; otherwise converts to 8-bit.
        /// </summary>
        Anydepth = 2,
        /// <summary>
        /// Loads the image in any possible color format.
        /// </summary>
        Anycolor = 4,
        /// <summary>
        /// Uses GDAL driver to load the image.
        /// </summary>
        LoadGdal = 8,
        /// <summary>
        /// Reduced grayscale2 option.
        /// </summary>
        ReducedGrayscale2 = 16,
        /// <summary>
        /// Reduced color2 option.
        /// </summary>
        ReducedColor2 = 17,
        /// <summary>
        /// Reduced grayscale4 option.
        /// </summary>
        ReducedGrayscale4 = 32,
        /// <summary>
        /// Reduced color4 option.
        /// </summary>
        ReducedColor4 = 33,
        /// <summary>
        /// Reduced grayscale8 option.
        /// </summary>
        ReducedGrayscale8 = 64,
        /// <summary>
        /// Reduced color8 option.
        /// </summary>
        ReducedColor8 = 65,
        /// <summary>
        /// Do not rotate the image according to EXIF orientation flag.
        /// </summary>
        IgnoreOrientation = 128,
        /// <summary>
        /// Color rgb option.
        /// </summary>
        ColorRgb = 256,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite B M P Compression Flags.
    /// </summary>
    public enum ImwriteBMPCompressionFlags : int
    {
        /// <summary>
        /// Rgb option.
        /// </summary>
        Rgb = 0,
        /// <summary>
        /// Bitfields option.
        /// </summary>
        Bitfields = 3,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite E X R Compression Flags.
    /// </summary>
    public enum ImwriteEXRCompressionFlags : int
    {
        /// <summary>
        /// No option.
        /// </summary>
        No = 0,
        /// <summary>
        /// Rle option.
        /// </summary>
        Rle = 1,
        /// <summary>
        /// Zips option.
        /// </summary>
        Zips = 2,
        /// <summary>
        /// Zip option.
        /// </summary>
        Zip = 3,
        /// <summary>
        /// Piz option.
        /// </summary>
        Piz = 4,
        /// <summary>
        /// Pxr24 option.
        /// </summary>
        Pxr24 = 5,
        /// <summary>
        /// B44 option.
        /// </summary>
        B44 = 6,
        /// <summary>
        /// B44a option.
        /// </summary>
        B44a = 7,
        /// <summary>
        /// Dwaa option.
        /// </summary>
        Dwaa = 8,
        /// <summary>
        /// Dwab option.
        /// </summary>
        Dwab = 9,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite E X R Type Flags.
    /// </summary>
    public enum ImwriteEXRTypeFlags : int
    {
        /// <summary>
        /// Half option.
        /// </summary>
        Half = 1,
        /// <summary>
        /// Float option.
        /// </summary>
        Float = 2,
    }

    /// <summary>
    /// Specifies encoding parameter flags for writing image files.
    /// </summary>
    public enum ImwriteFlags : int
    {
        /// <summary>
        /// Jpeg quality option.
        /// </summary>
        JpegQuality = 1,
        /// <summary>
        /// Jpeg progressive option.
        /// </summary>
        JpegProgressive = 2,
        /// <summary>
        /// Jpeg optimize option.
        /// </summary>
        JpegOptimize = 3,
        /// <summary>
        /// Jpeg rst interval option.
        /// </summary>
        JpegRstInterval = 4,
        /// <summary>
        /// Jpeg luma quality option.
        /// </summary>
        JpegLumaQuality = 5,
        /// <summary>
        /// Jpeg chroma quality option.
        /// </summary>
        JpegChromaQuality = 6,
        /// <summary>
        /// Jpeg sampling factor option.
        /// </summary>
        JpegSamplingFactor = 7,
        /// <summary>
        /// Png compression option.
        /// </summary>
        PngCompression = 16,
        /// <summary>
        /// Png strategy option.
        /// </summary>
        PngStrategy = 17,
        /// <summary>
        /// Png bilevel option.
        /// </summary>
        PngBilevel = 18,
        /// <summary>
        /// Png filter option.
        /// </summary>
        PngFilter = 19,
        /// <summary>
        /// Png zlibbuffer size option.
        /// </summary>
        PngZlibbufferSize = 20,
        /// <summary>
        /// Pxm binary option.
        /// </summary>
        PxmBinary = 32,
        /// <summary>
        /// Exr type option.
        /// </summary>
        ExrType = unchecked((int)(3 << 4 + 0)),
        /// <summary>
        /// Exr compression option.
        /// </summary>
        ExrCompression = unchecked((int)(3 << 4 + 1)),
        /// <summary>
        /// Exr dwa compression level option.
        /// </summary>
        ExrDwaCompressionLevel = unchecked((int)(3 << 4 + 2)),
        /// <summary>
        /// Webp quality option.
        /// </summary>
        WebpQuality = 64,
        /// <summary>
        /// Webp lossless mode option.
        /// </summary>
        WebpLosslessMode = 65,
        /// <summary>
        /// Hdr compression option.
        /// </summary>
        HdrCompression = unchecked((int)(5 << 4 + 0)),
        /// <summary>
        /// Pam tupletype option.
        /// </summary>
        PamTupletype = 128,
        /// <summary>
        /// Tiff resunit option.
        /// </summary>
        TiffResunit = 256,
        /// <summary>
        /// Tiff xdpi option.
        /// </summary>
        TiffXdpi = 257,
        /// <summary>
        /// Tiff ydpi option.
        /// </summary>
        TiffYdpi = 258,
        /// <summary>
        /// Tiff compression option.
        /// </summary>
        TiffCompression = 259,
        /// <summary>
        /// Tiff rowsperstrip option.
        /// </summary>
        TiffRowsperstrip = 278,
        /// <summary>
        /// Tiff predictor option.
        /// </summary>
        TiffPredictor = 317,
        /// <summary>
        /// Jpeg2000 compression x1000 option.
        /// </summary>
        Jpeg2000CompressionX1000 = 272,
        /// <summary>
        /// Avif quality option.
        /// </summary>
        AvifQuality = 512,
        /// <summary>
        /// Avif depth option.
        /// </summary>
        AvifDepth = 513,
        /// <summary>
        /// Avif speed option.
        /// </summary>
        AvifSpeed = 514,
        /// <summary>
        /// Jpegxl quality option.
        /// </summary>
        JpegxlQuality = 640,
        /// <summary>
        /// Jpegxl effort option.
        /// </summary>
        JpegxlEffort = 641,
        /// <summary>
        /// Jpegxl distance option.
        /// </summary>
        JpegxlDistance = 642,
        /// <summary>
        /// Jpegxl decoding speed option.
        /// </summary>
        JpegxlDecodingSpeed = 643,
        /// <summary>
        /// Bmp compression option.
        /// </summary>
        BmpCompression = 768,
        /// <summary>
        /// Gif loop option.
        /// </summary>
        GifLoop = 1024,
        /// <summary>
        /// Gif speed option.
        /// </summary>
        GifSpeed = 1025,
        /// <summary>
        /// Gif quality option.
        /// </summary>
        GifQuality = 1026,
        /// <summary>
        /// Gif dither option.
        /// </summary>
        GifDither = 1027,
        /// <summary>
        /// Gif transparency option.
        /// </summary>
        GifTransparency = 1028,
        /// <summary>
        /// Gif colortable option.
        /// </summary>
        GifColortable = 1029,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite G I F Compression Flags.
    /// </summary>
    public enum ImwriteGIFCompressionFlags : int
    {
        /// <summary>
        /// Fast no dither option.
        /// </summary>
        FastNoDither = 1,
        /// <summary>
        /// Fast floyd dither option.
        /// </summary>
        FastFloydDither = 2,
        /// <summary>
        /// Colortable size8 option.
        /// </summary>
        ColortableSize8 = 3,
        /// <summary>
        /// Colortable size16 option.
        /// </summary>
        ColortableSize16 = 4,
        /// <summary>
        /// Colortable size32 option.
        /// </summary>
        ColortableSize32 = 5,
        /// <summary>
        /// Colortable size64 option.
        /// </summary>
        ColortableSize64 = 6,
        /// <summary>
        /// Colortable size128 option.
        /// </summary>
        ColortableSize128 = 7,
        /// <summary>
        /// Colortable size256 option.
        /// </summary>
        ColortableSize256 = 8,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite H D R Compression Flags.
    /// </summary>
    public enum ImwriteHDRCompressionFlags : int
    {
        /// <summary>
        /// None option.
        /// </summary>
        None = 0,
        /// <summary>
        /// Rle option.
        /// </summary>
        Rle = 1,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite J P E G Sampling Factor Params.
    /// </summary>
    public enum ImwriteJPEGSamplingFactorParams : int
    {
        /// <summary>
        /// _411 option.
        /// </summary>
        _411 = unchecked((int)(0x411111)),
        /// <summary>
        /// _420 option.
        /// </summary>
        _420 = unchecked((int)(0x221111)),
        /// <summary>
        /// _422 option.
        /// </summary>
        _422 = unchecked((int)(0x211111)),
        /// <summary>
        /// _440 option.
        /// </summary>
        _440 = unchecked((int)(0x121111)),
        /// <summary>
        /// _444 option.
        /// </summary>
        _444 = unchecked((int)(0x111111)),
    }

    /// <summary>
    /// Specifies options and constants for Imwrite P A M Flags.
    /// </summary>
    public enum ImwritePAMFlags : int
    {
        /// <summary>
        /// Null option.
        /// </summary>
        Null = 0,
        /// <summary>
        /// Blackandwhite option.
        /// </summary>
        Blackandwhite = 1,
        /// <summary>
        /// Loads the image as a single-channel grayscale image.
        /// </summary>
        Grayscale = 2,
        /// <summary>
        /// Grayscale alpha option.
        /// </summary>
        GrayscaleAlpha = 3,
        /// <summary>
        /// Rgb option.
        /// </summary>
        Rgb = 4,
        /// <summary>
        /// Rgb alpha option.
        /// </summary>
        RgbAlpha = 5,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite P N G Filter Flags.
    /// </summary>
    public enum ImwritePNGFilterFlags : int
    {
        /// <summary>
        /// Filter none option.
        /// </summary>
        FilterNone = 8,
        /// <summary>
        /// Filter sub option.
        /// </summary>
        FilterSub = 16,
        /// <summary>
        /// Filter up option.
        /// </summary>
        FilterUp = 32,
        /// <summary>
        /// Filter avg option.
        /// </summary>
        FilterAvg = 64,
        /// <summary>
        /// Filter paeth option.
        /// </summary>
        FilterPaeth = 128,
        /// <summary>
        /// Fast filters option.
        /// </summary>
        FastFilters = unchecked((int)(FilterNone | FilterSub | FilterUp)),
        /// <summary>
        /// All filters option.
        /// </summary>
        AllFilters = unchecked((int)(FastFilters | FilterAvg | FilterPaeth)),
    }

    /// <summary>
    /// Specifies options and constants for Imwrite P N G Flags.
    /// </summary>
    public enum ImwritePNGFlags : int
    {
        /// <summary>
        /// Default option (equivalent to Reflect101 or basic default).
        /// </summary>
        Default = 0,
        /// <summary>
        /// Filtered option.
        /// </summary>
        Filtered = 1,
        /// <summary>
        /// Huffman only option.
        /// </summary>
        HuffmanOnly = 2,
        /// <summary>
        /// Rle option.
        /// </summary>
        Rle = 3,
        /// <summary>
        /// Fixed option.
        /// </summary>
        Fixed = 4,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite Tiff Compression Flags.
    /// </summary>
    public enum ImwriteTiffCompressionFlags : int
    {
        /// <summary>
        /// None option.
        /// </summary>
        None = 1,
        /// <summary>
        /// Ccittrle option.
        /// </summary>
        Ccittrle = 2,
        /// <summary>
        /// Ccittfax3 option.
        /// </summary>
        Ccittfax3 = 3,
        /// <summary>
        /// Ccitt t4 option.
        /// </summary>
        CcittT4 = 3,
        /// <summary>
        /// Ccittfax4 option.
        /// </summary>
        Ccittfax4 = 4,
        /// <summary>
        /// Ccitt t6 option.
        /// </summary>
        CcittT6 = 4,
        /// <summary>
        /// Lzw option.
        /// </summary>
        Lzw = 5,
        /// <summary>
        /// Ojpeg option.
        /// </summary>
        Ojpeg = 6,
        /// <summary>
        /// Jpeg option.
        /// </summary>
        Jpeg = 7,
        /// <summary>
        /// T85 option.
        /// </summary>
        T85 = 9,
        /// <summary>
        /// T43 option.
        /// </summary>
        T43 = 10,
        /// <summary>
        /// Next option.
        /// </summary>
        Next = 32766,
        /// <summary>
        /// Ccittrlew option.
        /// </summary>
        Ccittrlew = 32771,
        /// <summary>
        /// Packbits option.
        /// </summary>
        Packbits = 32773,
        /// <summary>
        /// Thunderscan option.
        /// </summary>
        Thunderscan = 32809,
        /// <summary>
        /// It8ctpad option.
        /// </summary>
        It8ctpad = 32895,
        /// <summary>
        /// It8lw option.
        /// </summary>
        It8lw = 32896,
        /// <summary>
        /// It8mp option.
        /// </summary>
        It8mp = 32897,
        /// <summary>
        /// It8bl option.
        /// </summary>
        It8bl = 32898,
        /// <summary>
        /// Pixarfilm option.
        /// </summary>
        Pixarfilm = 32908,
        /// <summary>
        /// Pixarlog option.
        /// </summary>
        Pixarlog = 32909,
        /// <summary>
        /// Deflate option.
        /// </summary>
        Deflate = 32946,
        /// <summary>
        /// Adobe deflate option.
        /// </summary>
        AdobeDeflate = 8,
        /// <summary>
        /// Dcs option.
        /// </summary>
        Dcs = 32947,
        /// <summary>
        /// Jbig option.
        /// </summary>
        Jbig = 34661,
        /// <summary>
        /// Sgilog option.
        /// </summary>
        Sgilog = 34676,
        /// <summary>
        /// Sgilog24 option.
        /// </summary>
        Sgilog24 = 34677,
        /// <summary>
        /// Jp2000 option.
        /// </summary>
        Jp2000 = 34712,
        /// <summary>
        /// Lerc option.
        /// </summary>
        Lerc = 34887,
        /// <summary>
        /// Lzma option.
        /// </summary>
        Lzma = 34925,
        /// <summary>
        /// Zstd option.
        /// </summary>
        Zstd = 50000,
        /// <summary>
        /// Webp option.
        /// </summary>
        Webp = 50001,
        /// <summary>
        /// Jxl option.
        /// </summary>
        Jxl = 50002,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite Tiff Predictor Flags.
    /// </summary>
    public enum ImwriteTiffPredictorFlags : int
    {
        /// <summary>
        /// None option.
        /// </summary>
        None = 1,
        /// <summary>
        /// Horizontal option.
        /// </summary>
        Horizontal = 2,
        /// <summary>
        /// Floatingpoint option.
        /// </summary>
        Floatingpoint = 3,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite Tiff Resolution Unit Flags.
    /// </summary>
    public enum ImwriteTiffResolutionUnitFlags : int
    {
        /// <summary>
        /// None option.
        /// </summary>
        None = 1,
        /// <summary>
        /// Inch option.
        /// </summary>
        Inch = 2,
        /// <summary>
        /// Centimeter option.
        /// </summary>
        Centimeter = 3,
    }

    /// <summary>
    /// Specifies options and constants for Imwrite W E B P Lossless Mode.
    /// </summary>
    public enum ImwriteWEBPLosslessMode : int
    {
        /// <summary>
        /// Off option.
        /// </summary>
        Off = 0,
        /// <summary>
        /// On option.
        /// </summary>
        On = 1,
        /// <summary>
        /// Preserve color option.
        /// </summary>
        PreserveColor = 2,
    }

}
