...
namespace OpenCV5Sharp.Samples.Maui.Cpu
{
    ...
    public class MainPage : ContentPage
    {
        ...
        public MainPage()
        {
            ...
            Title = "OpenCV5Sharp CPU MAUI Dashboard";
            ...
            {
                Text = "OpenCV5Sharp CPU Web/App Dashboard",
            ...
            // Filter Selector
            panelLayout.Children.Add(new Label { Text = "CPU Filter:", TextColor = Colors.Gray, FontSize = 12 });
            _filterPicker = new Picker { Title = "Select Filter", TextColor = Colors.White, TitleColor = Colors.Gray };
            _filterPicker.Items.Add("None");
            _filterPicker.Items.Add("Grayscale");
            _filterPicker.Items.Add("Canny Edge Detection");
            _filterPicker.Items.Add("Gaussian Blur");
            _filterPicker.Items.Add("YOLOv8 Object Detector (CPU)");
            _filterPicker.Items.Add("YuNet Face Tracker (CPU)");
            _filterPicker.Items.Add("Hand & Finger Tracker");
            _filterPicker.SelectedIndex = 0;
            _filterPicker.SelectedIndexChanged += FilterPicker_SelectedIndexChanged;
            panelLayout.Children.Add(_filterPicker);
            ...
            _timeLabel = new Label { Text = "Proc Time: N/A", TextColor = Color.FromArgb("#10b981"), FontSize = 13 };
            _matsLabel = new Label { Text = "Backend: OpenCV 5 CPU (C#)", TextColor = Colors.Gray, FontSize = 12 };
            panelLayout.Children.Add(_fpsLabel);
            panelLayout.Children.Add(_timeLabel);
            panelLayout.Children.Add(_matsLabel);
        ...
        private async Task StartCaptureLoopAsync(string? sourcePath)
        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    using var processed = ApplyFilter(frame);
                    ...
                    MainThread.BeginInvokeOnMainThread(() =>
                    {
                        ...
                        _timeLabel.Text = $"Proc Time: {elapsed} ms";
                    });
        ...
        private async Task ProcessStaticImageAsync(string path)
        {
            try
            {
                ...
                using var processed = ApplyFilter(src);
                byte[] jpegBytes = Cv2.Imencode(".jpg", processed);

                _imageControl.Source = ImageSource.FromStream(() => new MemoryStream(jpegBytes));
                _fpsLabel.Text = "FPS: N/A";
                _timeLabel.Text = "Proc Time: Static";
            }
            catch (Exception ex)
            {
        ...
        private Mat ApplyFilter(Mat src)
        {
        ...
        private async Task InitializeSelectedModelAsync()
        {
            ...
            if (_filterPicker.SelectedIndex == 4)
            {
            ...
            else if (_filterPicker.SelectedIndex == 5)
            {
                await DownloadModelIfMissingAsync(yunetPath, YunetUrl);
                if (_yunetNet == null)
                {
                    _yunetNet = FaceDetectorYN.Create(yunetPath, "", new Size(320, 320), 0.6f, 0.3f, 5000, 0, 0)!;
                }
            }
        }
        ...
        private async Task DownloadModelIfMissingAsync(string fullPath, string url)
        {
            ...
            MainThread.BeginInvokeOnMainThread(() => _matsLabel.Text = "Downloading weights...");
            using var client = new HttpClient();
            byte[] data = await client.GetByteArrayAsync(url);
            await File.WriteAllBytesAsync(fullPath, data);
            MainThread.BeginInvokeOnMainThread(() => _matsLabel.Text = "Weights Ready");
        }
